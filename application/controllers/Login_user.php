...
class Login_user extends CI_Controller {
  ...
  public function index()
  {
    ...
    if ($this->form_validation->run() == FALSE) {
      ...
      $this->load->view('auth/login_user/index', $data, FALSE);
    ...
    if ($user) {
      if ($user['user_active'] == 1) {
        ...
        if (password_verify($password, $user['password'])) {
          ...
          if ($user['user_id'] == 2) {
            # code...
            $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">Selamat datang di dashboard user!</div>');
            
            redirect('backend_user/dashboard');
